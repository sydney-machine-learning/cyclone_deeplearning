# -*- coding: utf-8 -*-
"""windspeed_multivariate.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1iPXfF4ql-ZOweAUrpwC1HruagYpvrtqy
"""
...
lati = array(df['latitude'])
speed = array(df['speed'])
longi = longi.reshape(len(longi),1)
lati = lati.reshape(len(lati),1)
speed = speed.reshape(len(speed),1)
longi.shape, lati.shape, speed.shape

dataset = hstack((longi, lati, speed))
dataset
...
longi_t = array(df['longitude'])
lati_t = array(df['latitude'])
speed_t = array(df['speed'])
longi_t = longi_t.reshape(len(longi_t),1)
lati_t = lati_t.reshape(len(lati_t),1)
speed_t = speed_t.reshape(len(speed_t),1)
dataset_t = hstack((longi_t, lati_t, speed_t))
X_TEST, Y_TEST = msplit_sequence(dataset_t, n_steps_in, n_steps_out)
